...
export default function PatientTable({
  ...
  useEffect(() => {
    setPage(1);
    setOpenActionId("");
  }, [query, category, followUp, sortBy]);
  ...
  const visiblePatients = useMemo(() => {
    ...
    const filtered = patients.filter((patient) => {
      ...
      const matchesFollowUp =
        followUp === "All" ||
        followUpState(patient) === followUp;
  ...
  const pageCount = Math.max(
    1,
    Math.ceil(
      visiblePatients.length / PAGE_SIZE,
    ),
  );
  ...
  const paginatedPatients =
    visiblePatients.slice(
      (currentPage - 1) * PAGE_SIZE,
      currentPage * PAGE_SIZE,
    );

  const showingFrom = visiblePatients.length
                ...
                (patient) => {
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <Link
                              ...
                              className="truncate text-sm font-semibold text-slate-700 transition hover:text-teal-700"
                            >
                              {patient.fullName}
                            </Link>
                      ...
                      <td className="px-4 py-4">
                        <p
                          className={`text-sm font-semibold ${followUpClasses(
                            state,
                          )}`}
                        >
                          {patient.nextFollowUp
                      ...
                      <td className="relative px-6 py-4">
                        ...
                        {menuOpen && (
                          <div className="absolute right-6 top-14 z-20 w-52 rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl shadow-slate-200/60">
                            <Link
                              href={`/patients/${patient.id}`}
                            ...
                            {whatsapp && (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="block rounded-lg px-3 py-2 text-sm font-medium text-slate-600 transition hover:bg-emerald-50 hover:text-emerald-700"
                              >
                                Message on WhatsApp
                              </a>
                            )}
