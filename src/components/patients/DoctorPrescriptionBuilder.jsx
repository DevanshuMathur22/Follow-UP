...
export default function DoctorPrescriptionBuilder({
  ...
  const prescriptionComparison = previousPrescription
    ? (() => {
        ...
        const previousList = previousMedicines
          .filter((item) => normalize(item?.medicine))
          .map(medicineKey);

        const currentList = medicines
          .filter((item) => normalize(item?.medicine))
          .map(medicineKey);

        return {
          diagnosisChanged:
            normalize(form.diagnosis) !==
            normalize(previousPrescription.diagnosis),
          previousMedicineCount: previousList.length,
          currentMedicineCount: currentList.length,
          medicinesChanged:
            ...
            {previousPrescription && (
              <div className="mt-3 rounded-lg border border-emerald-100 bg-emerald-50 px-3 py-2.5">
                <p className="text-xs font-semibold text-emerald-800">
                  Using prescription from{" "}
                  {new Date(
                    previousPrescription.issuedAt ||
                      previousPrescription.visitDate ||
                      previousPrescription.createdAt,
                ...
                <p className="mt-1 text-[11px] text-emerald-700">
                  {previousPrescription.doctor
                    ? `${previousPrescription.doctor} · `
                    : ""}
                  {previousPrescription.medicines?.length || 0} medicine
                  {(previousPrescription.medicines?.length || 0) === 1
                    ? ""
                    : "s"}
            ...
            {prescriptionComparison && (
              <div className="mt-2 flex flex-wrap gap-2 text-[11px] font-semibold">
                <span className={`rounded-md px-2 py-1 ${
                  prescriptionComparison.diagnosisChanged
                    ? "bg-amber-50 text-amber-700"
                    : "bg-slate-100 text-slate-600"
                }`}>
                  Diagnosis: {prescriptionComparison.diagnosisChanged
                    ? "Changed"
                    : "Unchanged"}
                </span>

                <span className={`rounded-md px-2 py-1 ${
                  prescriptionComparison.medicinesChanged
                    ? "bg-amber-50 text-amber-700"
                    : "bg-slate-100 text-slate-600"
                }`}>
                  Medicines: {prescriptionComparison.previousMedicineCount}
                  {" → "}
                  {prescriptionComparison.currentMedicineCount}
                  {prescriptionComparison.medicinesChanged
                    ? " · Changed"
                    : " · Unchanged"}
                </span>
